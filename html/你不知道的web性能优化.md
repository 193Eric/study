...
自从SPA火起来开始，我们前端基本都是用webpack来打包项目了，所以对网页优化的方面，我们除了对现有代码的优化，还有打包出来的代码结构的优化。上面我们说过，如果首页第一次加载的文件的文件过大，会导致页面白屏，及时用了一些技术避免白屏，但是首页还是会加载很慢。由于我们的文件都是通过webpack打包生成的，那我们能不能对第一次需要加载的文件进行一系列的优化和缩减呢？答案是当然可以的。同时我们还能通过webpack进行一些缓存命中处理。
